...
# Function to handle the button click for entity names
...
# Function to handle the button click for Tesla
def on_tesla_button_click():
    api_key = st.secrets["OPENAI_API_KEY"]
    prompt = "Tell me something about 特斯拉"
    result = query_chatgpt(prompt, api_key)
    st.write("API Response:", result)  # Log the entire API response for debugging
    choices = result.get("choices", [])
    if choices:
        ...
        description = "No description found."
    st.write(f"Description of 特斯拉: {description}")

# Add a button to trigger the search for Tesla
if st.button('Search for 特斯拉'):
    on_tesla_button_click()
...
def display_results_with_buttons(results):
    if 'CLOUD_FRONT_URL' in results.columns and 'MOVIE_FILE_IN_S3' in results.columns:
        ...
        for index, row in results.iterrows():
            ...
            button_key = f"button_{index}_{row['ENTITY_NAME'].replace(' ', '_')}_{index}"
